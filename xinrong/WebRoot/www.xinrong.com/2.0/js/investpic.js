	...
	var a={ 
		...
		images:function(){ 
			a.obj.find("img").bind("click",function(){ 
				...
				$("#intro").html(cont);
				$("#spec-n1 img").attr("src",src.replace("d","d"));
				a.posi();
		...
		swith:function(){ 
			$("#foward").bind("click",function(){ 
				...
				if(m[1]>0){ 
					...
					$("#intro").html(cont);
					$("#spec-n1").find("img").attr("src",src.replace("d","d"));
					a.posi();
		...
		roll:{ 
			foward:function(step){ 
				var m=a.calculate.roll();
				a.obj.find("ul").animate({ "marginTop":m[0]-152*step},100);
			},
			next:function(step){ 
				var m=a.calculate.roll();
				a.obj.find("ul").animate({ "marginTop":m[0]+152*step},100)		
		...
		init:function(){ 
			...
			a.obj.find("ul").css({ "height":h*152});
			a.images();
			a.swith();
			a.list();
			a.alpha();
