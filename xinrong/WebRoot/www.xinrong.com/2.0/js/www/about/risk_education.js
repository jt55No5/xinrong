...
function load_media_reports(pIndex,pageNum,callback){
	...
	$.ajax({
		...
		type:'GET',
		data:{pageSize:pageNum,pageIndex:pIndex,type:2},
		dataType:'json',
		success:function(result){
			...
			for(var i=0;i<rs.length;i++){
				...
				if(598 == rs[i].id){
					html+='<dd><h2><a href="https://www.xinrong.com/2.0/action/zt_315/zt_315.html" target=_blank>'+rs[i].title+'</a></h2>'+content+'<div class="d_font"><span class="left"><img src="'+logoImage+'" width="71" height="35" />&nbsp;</span><span class="right">'+AA.Helper.date(rs[i].ctimeLong/1000,'Y-m-d')+'</span></div></dd>';
				} else {
...
function load_media_report_detail(id){
	...
	if(id!=null&&id!=undefined&&id!=0){
		...
		if(isOuter){
			...
			var url = "/2.0/views/about/risk_education.shtml?di="+id;
			var state = {url:url,showList:false};
			history.replaceState(state,"信融财富风险教育活动",url);
		}
	}
}
...
function toggle_list_detail(showList) {
	if(showList){
		...
		var url = "/2.0/views/about/risk_education.shtml";
